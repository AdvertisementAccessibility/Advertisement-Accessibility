...
from results_utils import Actionables, AddressBook
from snapshot import Snapshot
from task.snapshot_task import SnapshotTask
from utils import annotate_elements
from adb_utils import disable_wifi
...
class ExtractActionsTask(SnapshotTask):
    ...
    async def execute(self):
        ...
        nodes_map = {}
        ad_queries = []
        ad_queries = [is_node_clickable]


        ad_queries.append(lambda  node: node.visible)
        ad_queries.append(lambda  node: node.is_ad)
        ...
        nodes_map[Actionables.Ad] = self.snapshot.get_nodes(filter_query=lambda  node: all(q(node) for q in ad_queries))
        ad_groups = self.group_elements_xpath(nodes_map[Actionables.Ad])
        tb_reachable_nodes = {}
        ...
        for mode in self.snapshot.address_book.extract_actions_modes:
            if mode == Actionables.Ad:
                for index, ad_group in enumerate(ad_groups):
                    with open(self.snapshot.address_book.audit_path_map[AddressBook.EXTRACT_ACTIONS].joinpath("Ad-actionables" + str(index)+ ".jsonl"), "w") as f:
                        for node in ad_group:
                            f.write(f"{node.toJSONStr()}\n")
                    annotate_elements(self.snapshot.initial_screenshot,
                                  self.snapshot.address_book.audit_path_map[AddressBook.EXTRACT_ACTIONS].joinpath("Ad-actionables" + str(index)+ ".png"),
                                  ad_group)
            unique_node_map = {}
    ...
    def group_elements_xpath(self, ad_list = List[Node]) -> List[Node]:
        similarity_threshold = 0.9

        groups = []

        for i, s1 in enumerate(ad_list):
            if s1 in [node for group in groups for node in group]:
                continue

            group = [s1]

            for s2 in ad_list[i + 1:]:
                if s2 in [node for group in groups for node in group]:
                    continue

                ratio = Levenshtein.ratio(s1.xpath, s2.xpath)
