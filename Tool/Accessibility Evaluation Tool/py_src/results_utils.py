...
class AddressBook:
    BASE_MODE = "base"
    FAROFF_MODE ="far_off"
    INEFFECTIVE_MODE = "ineffective"
    UNLOCATABLE_MODE = "unlocatable"
    INITIAL = "INITIAL"
    ## Audits
    ...
    def __init__(self, snapshot_result_path: Union[Path, str]):
        ...
        self.tb_explore_visited_nodes_gif = self.audit_path_map[AddressBook.TALKBACK_EXPLORE].joinpath(
            "visited_nodes.gif")
        # self.assertive_nodes = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('assertive_nodes_')
        self.tb_api_unvisited_nodes_screenshot = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('tb_api_unvisited_nodes.png')
        self.tb_touch_unvisited_nodes_screenshot = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('tb_touch_unvisited_nodes.png')
        self.tb_api_unlocatable_result = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('tb_api_result.jsonl')
        self.tb_api_unlocatable_gif = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('tb_api_visited_nodes.gif')
        self.tb_touch_unlocatable_result = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('tb_touch_result.jsonl')
        self.tb_touch_unlocatable_gif = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('tb_touch_visited_nodes.gif')
        self.unlabelled_screenshot = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('unlabelled_nodes.png')
        self.ineffective_screenshot = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('ineffective_nodes.png')
        self.far_off_elements = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('far_off_elements.png')
        self.swipe_num = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('swipe-nums.jsonl')
        self.ad_result = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('ad-result.jsonl')
        self.error_result = self.audit_path_map[AddressBook.UNLOCATABLE].joinpath('error-result.jsonl')
        ...
        self.single_ad_unit = {}
        for mode, value in self.extract_index.items():
            if mode == 0:
                self.single_ad_unit[mode] = self.audit_path_map[AddressBook.EXTRACT_ACTIONS].joinpath("Ad-actionables" + ".jsonl")
            else:
                self.single_ad_unit[mode] = self.audit_path_map[AddressBook.EXTRACT_ACTIONS].joinpath("Ad-actionables" + str(mode-1) + ".jsonl")
        ...
        self.execute_single_action_tb_focusables_gif_path = self.audit_path_map[AddressBook.EXECUTE_SINGLE_ACTION].joinpath(
            "tb_focusables.gif")
        # ---------------------------------------------------
        navigate_modes = [AddressBook.BASE_MODE, "tb_touch", "touch", "a11y_api", "tb_dir", "tb_jump", "tb_search", AddressBook.FAROFF_MODE, AddressBook.UNLOCATABLE_MODE, AddressBook.INEFFECTIVE_MODE]
        self.mode_path_map = {}
    ...
    def _get_path(self, mode: str, file_name_with_extension: str, should_exists: bool):
        if mode not in self.mode_path_map:
            return None
        if not self.mode_path_map[mode].exists():
            os.makedirs(self.mode_path_map[mode])
...
async def capture_current_state(address_book: AddressBook, device,
    ...
    if has_layout:
        ...
        logger.info("The layout path is: " + address_book.get_layout_path(mode,index).__str__())
